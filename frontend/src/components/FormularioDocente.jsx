...
function FormularioDocente({ onDocenteAdded, docenteToEdit, onCancel }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.dias_disponibles.length === 0) {
            setMessage('❌ Debe seleccionar al menos un día disponible')
            setLoading(false)
            return
        }
        ...
        try {
            ...
            if (response.ok) {
                setMessage(docenteToEdit ? '✅ Docente actualizado exitosamente' : '✅ Docente agregado exitosamente')
                setFormData({
                    nombre: '',
                    email: '',
                    numero: '',
                ...
                setMessage(`❌ Error: ${data.detail || `No se pudo ${docenteToEdit ? 'actualizar' : 'agregar'} el docente`}`)
            }
        } catch (error) {
        ...
        <form className="formulario-docente" onSubmit={handleSubmit}>
            ...
            <div className="form-actions">
                <button type="submit" className="btn-submit" disabled={loading}>
                    {loading ? '⏳ Guardando...' : (docenteToEdit ? 'Actualizar Docente' : 'Guardar Docente')}
                </button>
